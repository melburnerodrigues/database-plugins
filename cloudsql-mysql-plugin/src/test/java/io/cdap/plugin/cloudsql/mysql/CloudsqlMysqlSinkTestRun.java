...
public class CloudsqlMysqlSinkTestRun extends CloudsqlMysqlPluginTestBase {
  ...
  public void testDBSinkWithInvalidFieldLogicalType() throws Exception {
    testDBInvalidFieldLogicalType(
        "TIMESTAMP_COL", Schema.Type.LONG, getSinkConfig(), DATAPIPELINE_ARTIFACT);
  }

  @Test
  public void testDBSinkWithDBSchemaAndInvalidData() throws Exception {
    // MySQL JDBC connector allows to write integer values to STRING column. Use ENUM column
    // instead.
  ...
  private void testDBSink(String appName, String inputDatasetName, Schema schema) throws Exception {
    ...
    try (Connection conn = createConnection();
      ...
      for (StructuredRecord expected : inputRecords) {
        ...
        CustomAssertions.assertObjectEquals(
            expected.get("TINYTEXT_COL"), actual.getString("TINYTEXT_COL"));
        CustomAssertions.assertObjectEquals(
            expected.get("MEDIUMTEXT_COL"), actual.getString("MEDIUMTEXT_COL"));
        CustomAssertions.assertObjectEquals(
            expected.get("LONGTEXT_COL"), actual.getString("LONGTEXT_COL"));
        CustomAssertions.assertObjectEquals(
            expected.get("CHAR_COL"), actual.getString("CHAR_COL").trim());
        CustomAssertions.assertObjectEquals(
            expected.get("GRADUATED"), actual.getBoolean("GRADUATED"));
        ...
        CustomAssertions.assertObjectEquals(
            expected.get("MEDIUMINT_COL"), actual.getInt("MEDIUMINT_COL"));
        CustomAssertions.assertNumericEquals(expected.get("SCORE"), actual.getDouble("SCORE"));
        CustomAssertions.assertNumericEquals(
            expected.get("FLOAT_COL"), actual.getFloat("FLOAT_COL"));
        CustomAssertions.assertNumericEquals(
            expected.get("REAL_COL"), actual.getDouble("REAL_COL"));
        CustomAssertions.assertObjectEquals(
            expected.getDecimal("NUMERIC_COL"), actual.getBigDecimal("NUMERIC_COL"));
        CustomAssertions.assertObjectEquals(
            expected.getDecimal("DECIMAL_COL"), actual.getBigDecimal("DECIMAL_COL"));
        ...
        Assert.assertEquals(
            expected.getTime("TIME_COL").toSecondOfDay(),
        ...
        Assert.assertEquals(
            expected.getTimestamp("DATETIME_COL"),
            actual.getTimestamp("DATETIME_COL").toInstant().atZone(UTC_ZONE));
        Assert.assertEquals(
            expected.getTimestamp("TIMESTAMP_COL"),
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      StructuredRecord.Builder builder =
          StructuredRecord.builder(SCHEMA)
              ...
              .set("REAL_COL", 3.457)
              .setDecimal(
                  "NUMERIC_COL", new BigDecimal(3.458d, new MathContext(PRECISION)).setScale(SCALE))
              .setDecimal(
                  "DECIMAL_COL", new BigDecimal(3.459d, new MathContext(PRECISION)).setScale(SCALE))
              .set("BIT_COL", (i % 2 == 1))
