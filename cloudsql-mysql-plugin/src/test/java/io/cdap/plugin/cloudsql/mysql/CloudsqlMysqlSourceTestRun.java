...
public class CloudsqlMysqlSourceTestRun extends CloudsqlMysqlPluginTestBase {

  @Test
  @SuppressWarnings("ConstantConditions")
  public void testDBMacroSupport() throws Exception {
    String importQuery =
        "SELECT * FROM my_table WHERE DATE_COL <= '${logicalStartTime(yyyy-MM-dd,1d)}' "
            + "AND $CONDITIONS";
    String boundingQuery = "SELECT MIN(ID),MAX(ID) from my_table";
    String splitBy = "ID";

    ImmutableMap<String, String> sourceProps =
        ImmutableMap.<String, String>builder()
            .putAll(BASE_PROPS)
                .put(ConnectionConfig.CONNECTION_STRING, connectionUrl)
    ...
    ETLPlugin sourceConfig =
        new ETLPlugin(CloudsqlMysqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, sourceProps);

    ETLPlugin sinkConfig = MockSink.getPlugin("macroOutputTable");

    ApplicationManager applicationManager =
        deployETL(sourceConfig, sinkConfig, DATAPIPELINE_ARTIFACT, "testDBMacro");
    
    Map<String, String> args = new HashMap<>();
    args.put("logical.start.time", String.valueOf(CURRENT_TS));
//    args.put(ConnectionConfig.CONNECTION_STRING, connectionUrl);
  ...
  public void testDBSource() throws Exception {
    String importQuery =
        "SELECT ID, NAME, SCORE, GRADUATED, TINY, MEDIUMINT_COL, SMALL, BIG, FLOAT_COL, REAL_COL, NUMERIC_COL, "
            + "CHAR_COL, DECIMAL_COL, BIT_COL, BINARY_COL, DATE_COL, TIME_COL, DATETIME_COL, TIMESTAMP_COL, "
            + "VARBINARY_COL, BLOB_COL, MEDIUMBLOB_COL, TINYBLOB_COL, YEAR_COL, LONGBLOB_COL, TINYTEXT_COL, "
    ...
    ApplicationManager appManager =
        deployETL(sourceConfig, sinkConfig, DATAPIPELINE_ARTIFACT, "testDBSource");
    ...
    Assert.assertEquals(
        new BigDecimal(124.45, new MathContext(PRECISION)).setScale(SCALE),
        row1.getDecimal("NUMERIC_COL"));
    Assert.assertEquals(
        new BigDecimal(125.45, new MathContext(PRECISION)).setScale(SCALE),
    ...
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("BINARY_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("BINARY_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("VARBINARY_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("VARBINARY_COL")).array(), 0, 5));
    ...
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("MEDIUMBLOB_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("MEDIUMBLOB_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("TINYBLOB_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("TINYBLOB_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("LONGBLOB_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("LONGBLOB_COL")).array(), 0, 5));
  }

  @Test
  public void testDbSourceMultipleTables() throws Exception {
    String importQuery =
        "SELECT my_table.ID, your_table.NAME FROM my_table, your_table "
            + "WHERE my_table.ID < 3 and my_table.ID = your_table.ID and $CONDITIONS ";
    String boundingQuery =
        "SELECT LEAST(MIN(my_table.ID), MIN(your_table.ID)), "
    ...
    ApplicationManager appManager =
        deployETL(
            sourceConfig, sinkConfig, DATAPIPELINE_ARTIFACT, "testDBSourceWithMultipleTables");
    runETLOnce(appManager);
  ...
  public void testUserNamePasswordCombinations() throws Exception {
    ...
    Map<String, String> baseSourceProps =
        ImmutableMap.<String, String>builder()
    ...
    ETLPlugin dbConfig =
        new ETLPlugin(
            CloudsqlMysqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, baseSourceProps, null);
    ...
    database =
        new ETLStage(
            "databaseSource",
            new ETLPlugin(
                CloudsqlMysqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, noUser, null));
    etlConfig =
        ETLBatchConfig.builder()
            .addStage(database)
            .addStage(table)
    ...
    database =
        new ETLStage(
            "databaseSource",
            new ETLPlugin(
                CloudsqlMysqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, emptyPassword, null));
    etlConfig =
        ETLBatchConfig.builder()
            .addStage(database)
            .addStage(table)
