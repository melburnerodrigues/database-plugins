...
public class CloudsqlPostgresPluginTestBase extends DatabasePluginTestBase {
  ...
  protected static final ArtifactSummary DATAPIPELINE_ARTIFACT =
      new ArtifactSummary("data-pipeline", "3.5.0");
  protected static final long CURRENT_TS = System.currentTimeMillis();

  protected static final String JDBC_DRIVER_NAME = "cloudsql-postgresql";
  ...
  protected static final OffsetDateTime OFFSET_TIME =
      OffsetDateTime.of(1992, 3, 11, 12, 0, 10, 0, ZoneOffset.of("+03"));
  ...
  // TODO: REMOVE INSTANCE NAME AND OTHER SENSITIVE INFO
  protected static final Map<String, String> BASE_PROPS =
      ImmutableMap.<String, String>builder()
          .put(
              CloudsqlPostgresConstants.INSTANCE_NAME,
              System.getProperty(
                  "cloudsqlPostgresql.instanceName",
                  ""))
          .put(ConnectionConfig.DATABASE, System.getProperty("cloudsqlPostgresql.database", "mydb"))
          .put(ConnectionConfig.USER, System.getProperty("cloudsqlPostgresql.username", "postgres"))
          .put(
              ConnectionConfig.PASSWORD,
              System.getProperty("cloudsqlPostgresql.password", ""))
  ...
  public static void setupTest() throws Exception {
    ...
    addPluginArtifact(
        ...
        CloudsqlPostgresSource.class,
        CloudsqlPostgresSink.class,
        DBRecord.class,
        ETLDBOutputFormat.class,
        DataDrivenETLDBInputFormat.class,
        DBRecord.class,
        CloudsqlPostgresAction.class);

    PluginClass cloudsqlPostgresqlDriver =
        new PluginClass(
            ConnectionConfig.JDBC_PLUGIN_TYPE,
            JDBC_DRIVER_NAME,
            "cloudsql-postrgesql driver class",
            Driver.class.getName(),
            null,
            Collections.emptyMap());
    addPluginArtifact(
        NamespaceId.DEFAULT.artifact("postrgesql-jdbc-connector", "1.0.0"),
        DATAPIPELINE_ARTIFACT_ID,
        Sets.newHashSet(cloudsqlPostgresqlDriver),
        Driver.class);
  ...
  private static void populateData(PreparedStatement... stmts) throws SQLException {
    ...
    for (PreparedStatement pStmt : stmts) {
      for (int i = 1; i <= 5; i++) {
        ...
        pStmt.setObject(
            19, new PGTime(123456, Calendar.getInstance(TimeZone.getTimeZone(ZoneId.of("GMT+3")))));
