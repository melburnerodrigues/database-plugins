...
public class CloudsqlPostgresSourceTestRun extends CloudsqlPostgresPluginTestBase {
  ...
  public void testDBMacroSupport() throws Exception {
    ...
    ETLPlugin sourceConfig =
        new ETLPlugin(CloudsqlPostgresConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, sourceProps);
  ...
  public void testDBSource() throws Exception {
    ...
    ETLPlugin sourceConfig =
        new ETLPlugin(
            CloudsqlPostgresConstants.PLUGIN_NAME,
    ...
    Assert.assertEquals(
        expectedTs, row1.getTimestamp("TIMESTAMP_COL", ZoneId.ofOffset("UTC", ZoneOffset.UTC)));
    ...
    Assert.assertEquals(
        "user1", Bytes.toString(((ByteBuffer) row1.get("BYTEA_COL")).array(), 0, 5));
    Assert.assertEquals(
        "user2", Bytes.toString(((ByteBuffer) row2.get("BYTEA_COL")).array(), 0, 5));
  ...
  public void testDbSourceMultipleTables() throws Exception {
    ...
    String boundingQuery =
        "SELECT MIN(MIN(my_table.ID), MIN(your_table.ID)), "
  ...
  public void testUserNamePasswordCombinations() throws Exception {
    ...
    Map<String, String> baseSourceProps =
        ImmutableMap.<String, String>builder()
            .put(
                CloudsqlPostgresConstants.INSTANCE_NAME,
                BASE_PROPS.get(CloudsqlPostgresConstants.INSTANCE_NAME))
    ...
    ETLPlugin dbConfig =
        new ETLPlugin(
            CloudsqlPostgresConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, baseSourceProps, null);
    ...
    database =
        new ETLStage(
            "databaseSource",
            new ETLPlugin(
                CloudsqlPostgresConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, noUser, null));
    etlConfig =
        ETLBatchConfig.builder()
            .addStage(database)
            .addStage(table)
